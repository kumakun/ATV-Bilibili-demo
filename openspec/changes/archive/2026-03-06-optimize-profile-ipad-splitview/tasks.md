...
- [x] 7.3 测试从其他 Tab 切换回来时，iPad 侧边栏选中状态的保持
...
- [x] 9.1 在 iPhone（不同尺寸）上测试：验证移除统计数据后的布局和功能正常
- [x] 9.2 在 iPad（不同尺寸）上测试：验证 SplitView 布局和侧边栏交互正常
- [x] 9.3 测试 iPad 横竖屏切换：验证侧边栏自动收起和展开
- [x] 9.4 测试功能导航：验证每个功能项点击后正确显示详情页
- [x] 9.5 测试账号切换：验证切换后侧边栏用户信息更新，选中状态清除
- [x] 9.6 测试登出功能：验证确认对话框显示和登出操作正常
- [x] 9.7 测试状态保持：验证切换 Tab 后返回时的状态保持
- [x] 9.8 测试小屏 iPad（iPad mini）：验证侧边栏 overlay 模式正常工作
