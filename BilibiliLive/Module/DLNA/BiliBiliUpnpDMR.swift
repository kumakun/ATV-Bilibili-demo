...
class BiliBiliUpnpDMR: NSObject {
    ...
    func start() {
        ...
        httpServer["projection"] = nvasocket(
            uuid: bUuid,
            didConnect: { [weak self] session in
    ...
    func handlePlay(json: JSON) {
        ...
        if roomId.count > 0, let room = Int(roomId), room > 0 {
            ...
            // Ignore live room playback requests
            print("Live streaming feature not available (room: \(room))")
        } else {
            playVideo(json: json)
        }
    }
    ...
    @MainActor func sendStatus(status: PlayStatus) {
        ...
        for item in Array(sessions) {
            item.sendCommand(action: "OnPlayState", content: ["playState": status.rawValue])
        }
    }

    @MainActor func sendProgress(duration: Int, current: Int) {
        for item in Array(sessions) {
            item.sendCommand(action: "OnProgress", content: ["duration": duration, "position": current])
        }
    }
...
extension BiliBiliUpnpDMR {
    // Note: playLive method removed as live streaming feature has been removed
...
extension BiliBiliUpnpDMR: GCDAsyncUdpSocketDelegate {
    func udpSocket(
        _ sock: GCDAsyncUdpSocket, didReceive data: Data, fromAddress address: Data,
        ...
        address.withUnsafeBytes { (pointer: UnsafeRawBufferPointer) in
            ...
            guard
                getnameinfo(
                    unsafePtr, socklen_t(data.count), &hostname, socklen_t(hostname.count), nil, 0,
                    NI_NUMERICHOST
                ) == 0
            else {
                return
