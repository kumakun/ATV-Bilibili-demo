...
class SearchResultViewController: UIViewController {
  ...
  private func updateSnapshot(searchResult: SearchResult?, liveResult: SearchLiveResult?) {
    ...
    let defaultHeight = NSCollectionLayoutDimension.fractionalWidth(
      Settings.displayStyle == .large ? 0.26 : 0.2)
    ...
    if let searchResult {
      for section in searchResult.result {
        ...
        case .user(let data):
          let list = SearchList(
            title: "用户", height: .estimated(140), scrollingBehavior: .continuous)
          currentSnapshot.appendSections([list])
...
extension SearchResultViewController {
    ...
    let sectionProvider = {
      ...
      if sectionIdentifier.scrollingBehavior == .none {
        ...
        let itemSize = NSCollectionLayoutSize(
          widthDimension: sectionIdentifier.width,
          heightDimension: .fractionalHeight(1))
        let item = NSCollectionLayoutItem(layoutSize: itemSize)
        let hSpacing: CGFloat = Settings.displayStyle == .large ? 35 : 30
        item.contentInsets = NSDirectionalEdgeInsets(
          top: 0, leading: hSpacing, bottom: 0, trailing: hSpacing)
        let group = NSCollectionLayoutGroup.horizontal(layoutSize: groupSize, subitems: [item])
        group.edgeSpacing = .init(
          leading: .fixed(0), top: .fixed(40), trailing: .fixed(0), bottom: .fixed(-60))
        ...
        let group = NSCollectionLayoutGroup.horizontal(layoutSize: groupSize, subitems: [item])
        group.edgeSpacing = .init(
          leading: .fixed(0), top: .fixed(40), trailing: .fixed(0), bottom: .fixed(0))
        section = NSCollectionLayoutSection(group: group)
  ...
  private func configureDataSource() {
    ...
    let userCell = UICollectionView.CellRegistration<UpCell, SearchResult.User> {
      ...
      $0.imageView.kf.setImage(
        with: $2.upic.addSchemeIfNeed(),
        options: [
    ...
    dataSource = UICollectionViewDiffableDataSource<SearchList, Item>(
      ...
      case .video(let item):
        return collectionView.dequeueConfiguredReusableCell(
          using: displayCell, for: indexPath, item: item)
      case .bangumi(let item):
        return collectionView.dequeueConfiguredReusableCell(
          using: displayCell, for: indexPath, item: item)
      case .user(let item):
        return collectionView.dequeueConfiguredReusableCell(
          using: userCell, for: indexPath, item: item)
...
extension SearchResultViewController: UICollectionViewDelegate {
  func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    ...
    case .liveRoom:
      // Note: Live streaming feature has been removed
      // Live room search results will be displayed but not actionable
      break
    }
  }

  func indexPathForPreferredFocusedView(in collectionView: UICollectionView) -> IndexPath? {
    if let section = currentSnapshot.sectionIdentifiers.first,
...
extension SearchResultViewController: UISearchResultsUpdating {
  ...
  func updateSearchResults(
    for searchController: UISearchController, selecting searchSuggestion: any UISearchSuggestion
  ) {
    // 选中建议词后添加搜索历史
...
extension WebRequest {
  static func requestSearchResult(key: String) async throws -> SearchResult {
    try await request(
      url: "https://api.bilibili.com/x/web-interface/search/all/v2", parameters: ["keyword": key])
  }

  static func requestSearchLiveResult(key: String) async throws -> SearchLiveResult {
    try await request(
      url: "https://api.bilibili.com/x/web-interface/wbi/search/type",
      parameters: ["keyword": key, "search_type": "live"])
  }

  static func requestSuggest(key: String) async throws -> SuggestResult {
    try await request(
      url: "https://api.bilibili.com/x/web-interface/suggest", parameters: ["term": key])
  }
}
...
struct SearchResult: Decodable, Hashable {
  struct Video: Codable, Hashable, DisplayData {
    ...
    var overlay: DisplayOverlay? {
      ...
      leftItems.append(
        DisplayOverlay.DisplayOverlayItem(
          icon: "play.rectangle", text: play == 0 ? "-" : play?.numberString() ?? "-"))
      leftItems.append(
        DisplayOverlay.DisplayOverlayItem(
          icon: "list.bullet.rectangle", text: danmaku == 0 ? "-" : danmaku.numberString()))
      if let duration {
  ...
  enum Section: Decodable, Hashable {
    ...
    init(from decoder: Decoder) throws {
      let container: KeyedDecodingContainer<SearchResult.Section.CodingKeys> =
        try decoder.container(keyedBy: SearchResult.Section.CodingKeys.self)
